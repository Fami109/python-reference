    ...
    <div className={styles.page}>
      <div className={styles.hero}>
        ...
        <h1 className={styles.heroTitle}>
          初心者向けサイトには<br />
          <span className={styles.heroAccent}>載っていない</span> 実務の知識
        </h1>
        <p className={styles.heroDesc}>
          型システム・デコレータ・非同期・パフォーマンスチューニングまで。
          実務でPythonを書き続けるために必要なTipsをまとめたリファレンスサイト。
        </p>
        <div className={styles.stats}>
          <div className={styles.stat}>
            <span className={styles.statNum}>{sections.length}</span>
            <span className={styles.statLabel}>トピック</span>
          </div>
          ...
          <div className={styles.statDivider} />
          <div className={styles.stat}>
            <span className={styles.statNum}>{sections.reduce((acc, s) => acc + s.blocks.length, 0)}</span>
            <span className={styles.statLabel}>コードサンプル</span>
          </div>
        </div>
      </div>
            ...
            >
              <div className={styles.cardTop}>
                ...
                <span
                  ...
                  style={{ color: badgeColors[item.badgeColor] }}
                >
                  {item.badge}
                </span>
              </div>
