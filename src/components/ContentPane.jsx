    ...
    <article className={styles.pane}>
      <header className={styles.header}>
        ...
        <div className={styles.titleRow}>
          ...
          <span className={styles.badge} style={{ background: bc.bg, color: bc.color }}>{section.badge}</span>
        </div>
        <p className={styles.summary}>{section.summary}</p>
        {section.tips && section.tips.length > 0 && (
          <div className={styles.tipsBox}>
            <div className={styles.tipsHeader}><Lightbulb size={14} />実務Tips</div>
            <ul className={styles.tipsList}>
              {section.tips.map((tip, i) => <li key={i} className={styles.tip}>{tip}</li>)}
            </ul>
          </div>
        )}
      </header>
