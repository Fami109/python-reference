  ...
  return (
    <aside className={styles.sidebar}>
        ...
        {categories.map(cat => {
          ...
          return (
            <div key={cat} className={styles.group}>
              <button className={styles.groupHeader} onClick={() => toggle(cat)}>
                <Icon size={13} />
                <span>{cat}</span>
                {isCollapsed ? <ChevronRight size={12} className={styles.chevron} /> : <ChevronDown size={12} className={styles.chevron} />}
              </button>
              {!isCollapsed && items.map(item => (
                ...
                >
                  ...
                  <span className={styles.badge} style={{ color: badgeColors[item.badgeColor] || badgeColors.blue }}>
                    {item.badge}
                  </span>
                </button>
              ))}
      ...
      <div className={styles.footer}>
        <div className={styles.footerText}>Ryo's Python Reference</div>
      </div>
    </aside>
  )
}
